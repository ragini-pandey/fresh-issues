  ...
  return (
    <div className="flex items-center gap-1.5 sm:gap-2 text-sm text-muted-foreground shrink-0">
      ...
      {newCount > 0 && (
        <Badge
          variant="outline"
          className="border-primary/20 bg-primary/5 text-primary text-xs font-semibold px-2 py-0.5 animate-fade-in"
        >
          +{newCount} new
        </Badge>
      )}
      {rateLimit.remaining !== null && (
        <Tooltip>
          <TooltipTrigger asChild>
            <span
              className={`text-xs font-mono tabular-nums hidden sm:inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md ${
                rateLimit.remaining === 0
                  ? 'text-destructive bg-destructive/10 font-bold'
                  : rateLimit.remaining < 5
                    ? 'text-orange-600 dark:text-orange-500 bg-orange-500/10 font-semibold'
                    : 'text-muted-foreground/60'
              }`}
            >
              {isLowRateLimit && rateLimit.remaining > 0 && (
