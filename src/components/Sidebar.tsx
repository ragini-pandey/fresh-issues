...
export default function Sidebar({
  ...
  // Filter mutations are deprioritized — keeps button feedback snappy when
  // toggling many chips quickly.
  function applyFilter(partial: Partial<SearchFilters>) {
    startTransition(() => updateFilters(partial));
  }
  ...
  function toggleLabel(label: string) {
    ...
    const next = current.includes(label)
      ...
      : [...current, label];
    applyFilter({ labels: next });
  }

  const activeFilterCount = [
    filters.language ? 1 : 0,
    (filters.minStars ?? 0) > 0 ? 1 : 0,
    (filters.minComments ?? 0) > 0 ? 1 : 0,
            ...
            <div className="flex flex-wrap gap-1.5">
              {TIME_WINDOWS.map((tw) => (
                <button
                  key={tw.value}
                  type="button"
                  onClick={() => applyFilter({ timeWindow: tw.value })}
            ...
            <CollapsibleContent>
              <div className="space-y-4 pt-3">
                <FilterSection label="Sort By">
                  <div className="flex flex-wrap gap-1.5">
                    {SORT_OPTIONS.filter(
                      (opt) => !opt.reposOnly || repoCount > 0,
                    ).map((opt) => (
                      <button
                        key={opt.value}
                        type="button"
                        onClick={() =>
                          applyFilter({ sortBy: opt.value as SortBy })
                        }
                        className={`px-3 py-1.5 text-sm font-medium rounded-lg border transition-all cursor-pointer ${
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {STAR_PRESETS.map((preset) => (
                      <button
                        key={preset.value}
                        type="button"
                        onClick={() => applyFilter({ minStars: preset.value })}
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {COMMENT_PRESETS.map((preset) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          applyFilter({ minComments: preset.value })
                        }
                        className={`px-3 py-1.5 text-sm font-medium rounded-lg border transition-all cursor-pointer flex items-center gap-1 ${
                ...
                <div className="space-y-2.5 pt-1">
                  <SettingsToggle
                    checked={Boolean(filters.noAssignee)}
                    onCheckedChange={(v) => applyFilter({ noAssignee: v })}
              ...
              <div className="space-y-3 pt-3">
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-muted-foreground/70">
                    Increases rate limit to 30 req/min
                  </p>
                </div>
                <SettingsToggle
                  checked={sound}
