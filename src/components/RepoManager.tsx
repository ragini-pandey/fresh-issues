...
interface SortableRepoCardProps {
  ...
  cancelEdit: () => void;
  startEdit: (repo: TrackedRepo) => void;
  ...
  return (
    ...
    >
      <CardContent className="flex items-center gap-3 p-4">
        ...
        <Avatar
          className={`size-9 rounded-lg shrink-0 ${repo.disabled ? 'grayscale' : ''}`}
        >
          <AvatarImage
            src={`https://github.com/${repo.fullName.split('/')[0]}.png?size=64`}
          />
          <AvatarFallback className="rounded-lg text-sm">
            {repo.fullName.charAt(0).toUpperCase()}
          </AvatarFallback>
        </Avatar>
          ...
          <>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted-foreground">
                {repo.disabled
                  ? 'Disabled — not scanned for issues'
...
export default function RepoManager({
  ...
  function handleAdd(e: FormEvent<HTMLFormElement>) {
    ...
    if (
      repos.some(
        (r) => r.fullName.toLowerCase() === newRepo.trim().toLowerCase(),
      )
    ) {
  ...
  function startEdit(repo: TrackedRepo) {
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm sm:text-base text-muted-foreground">
              Add repos to track — only these will be searched for issues
            </p>
          </div>
        </div>
        ...
        {repos.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground animate-fade-in-up">
            ...
            <p className="text-sm text-muted-foreground mt-1.5 max-w-xs text-center leading-relaxed">
              Add repositories above to track their open issues. When repos
              are saved, only those repos will be searched.
            </p>
          </div>
        ) : (
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="text-sm">
                  {repos.filter((r) => !r.disabled).length} / {repos.length}{' '}
                  {repos.length === 1 ? 'repo' : 'repos'} active
                </Badge>
